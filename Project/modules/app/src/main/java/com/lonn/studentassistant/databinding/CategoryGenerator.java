...
public class CategoryGenerator {
	...
	public static <T extends GradeViewModel> List<CategoryViewModel<T>> gradeCourseCategories(CategoryViewModel<T> parentCategory,
																								List<CourseViewModel> courses) {
		List<CategoryViewModel<T>> subcategories = new ArrayList<>();

		for (CourseViewModel course : courses) {
			subcategories.add(new CategoryViewModel<T>()
					.setCategoryTitle(course.getName())
					.setEntityName(parentCategory.getEntityName())
					.setViewType(parentCategory.getViewType())
					.setPermissionLevel(parentCategory.getPermissionLevel())
					.setShowEmpty(parentCategory.isShowEmpty())
					.setShowHeader(true)
					.setShouldContain((grade) -> grade.getCourseKey().equals(course.getKey())));
		}
